...
export const ClusterPodItemBulkAssign = ({
  ...
  const toggleItemSelection = (item: InventoryItem, checked: boolean) => {
    ...
    if (!checked) {
      setSelectedItems((prev) => prev.filter((entry) => entry.item_id !== item.id))
      return
    }
  ...
  const updateSelectedItem = (
    ...
    setSelectedItems((prev) =>
      prev.map((entry) => (entry.item_id === itemId ? { ...entry, [key]: value } : entry))
    )
  }
  ...
  const validateAndBuildItems = (): ClusterBulkPodItemEntry[] | null => {
    ...
    for (const entry of selectedItems) {
      ...
      if (expected == null || current == null) {
        toast.error(`Invalid quantity for item ${entry.name}. Use non-negative integers only.`)
        return null
      }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(response.message || 'Assigned pod items for cluster successfully')
    } catch (error: any) {
      toast.error(error?.response?.data?.message || 'Failed to assign pod items for cluster')
    } finally {
    ...
    <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Select a pod cluster, choose multiple items, then assign expected/current quantities to all pods in that cluster.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">Pod Cluster</label>
            <select
              value={clusterId}
              onChange={(e) => {
            ...
            >
              <option value="">Select a pod cluster</option>
              {sortedClusters.map((cluster) => (
                <option key={cluster.id} value={cluster.id}>
                  {cluster.name} ({cluster.location?.name ?? cluster.location_id})
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            ...
            <div className="max-h-72 overflow-y-auto divide-y divide-gray-100">
              {isItemsLoading ? (
                <div className="px-4 py-6 text-sm text-gray-400">Loading items...</div>
              ) : filteredItems.length === 0 ? (
                <div className="px-4 py-6 text-sm text-gray-400">No items found</div>
              ) : (
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <div className="p-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              <span className="text-sm font-medium text-gray-800">Selected Item Quantities</span>
              <span className="text-xs text-gray-500">Non-negative integers only</span>
            </div>

            <div className="max-h-72 overflow-y-auto divide-y divide-gray-100">
              {selectedItems.length === 0 ? (
                <div className="px-4 py-6 text-sm text-gray-400">Select items to configure quantities</div>
              ) : (
                selectedItems.map((entry) => (
                  <div key={entry.item_id} className="px-4 py-3">
                    <p className="text-sm font-medium text-gray-900 mb-2">{entry.name}</p>
                    <div className="grid grid-cols-2 gap-3">
                      <input
                        ...
                        onChange={(e) => updateSelectedItem(entry.item_id, 'expected_quantity', e.target.value)}
                      ...
                      <input
                        ...
                        onChange={(e) => updateSelectedItem(entry.item_id, 'current_quantity', e.target.value)}
                        placeholder="Current"
                        className="px-3 py-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      />
                    </div>
        ...
        <div className="flex justify-end">
          ...
          >
            {isSubmitting ? 'Assigning...' : 'Assign Items To Cluster Pods'}
          </button>
        </div>
      </form>
      ...
      {summary && (
        <div className="mt-5 border border-gray-200 rounded-lg p-4 bg-slate-50">
          <h3 className="text-sm font-semibold text-gray-900 mb-3">Bulk Assignment Summary</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            ...
            <div className="bg-white border border-gray-200 rounded-lg px-3 py-2">
              <p className="text-gray-500">Total Pairs</p>
              <p className="font-semibold text-gray-900">{summary.total_target_pairs}</p>
            </div>
            <div className="bg-white border border-emerald-200 rounded-lg px-3 py-2">
              <p className="text-emerald-700">Created</p>
              <p className="font-semibold text-emerald-800">{summary.created_count}</p>
            </div>
            <div className="bg-white border border-amber-200 rounded-lg px-3 py-2">
              <p className="text-amber-700">Skipped Existing</p>
              <p className="font-semibold text-amber-800">{summary.skipped_existing_count}</p>
            </div>
          </div>
          ...
          {summary.created_count === 0 && (
            <div className="mt-3 px-3 py-2 border border-amber-200 rounded-lg bg-amber-50 text-amber-800 text-sm flex items-start gap-2">
              ...
              All selected pod-item pairs already existed. No new records were created.
            </div>
          )}
        </div>
      )}
