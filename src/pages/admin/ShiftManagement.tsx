      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editId ? 'Edit Shift' : 'Create Shift'} size="sm">
        <div className="space-y-4">
          ...
          <div className="pt-4 flex justify-end gap-2">
            <button disabled={isSaving} onClick={() => setIsModalOpen(false)} className="px-4 py-2 border rounded-lg text-gray-600 hover:bg-gray-50">
              Cancel
            </button>
            <button disabled={isSaving} onClick={handleSubmit} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
              Save
            </button>
          </div>
        </div>
    ...
    <div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Bind Shift to Location" size="sm">
        <div className="space-y-4">
          ...
          <div className="pt-4 flex justify-end gap-2">
            <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 border rounded-lg text-gray-600">
              Cancel
            </button>
    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-sm text-left">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              <th className="px-6 py-4 font-medium text-gray-500">Manager</th>
            ...
            {filteredAssignments.map((a) => {
              ...
              return (
                <tr key={a.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-medium text-gray-900">{manager?.name || 'Unknown'}</td>
