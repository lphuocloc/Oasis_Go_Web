...
import {
  getCheckInStateDetail,
  getOrderStatusDetail,
} from "../../utils/status";
import { formatBookingPeriod } from "../../utils/format";
...
export const BookingManagement = () => {
  ...
  const [orderPagination, setOrderPagination] =
    useState<BookingOrderPagination>({
      total: 0,
      page: 1,
      limit: 20,
      pages: 1,
  ...
  const [selectedOrderDetail, setSelectedOrderDetail] =
    useState<BookingOrderDetail | null>(null);
  const [selectedOrderIncidents, setSelectedOrderIncidents] = useState<
    OrderIncidentItem[]
  >([]);
  ...
  const [updatingCleanerAccessId, setUpdatingCleanerAccessId] = useState<
    string | null
  >(null);

  const scopedClusterIds = useMemo(
    () => new Set(clusters.map((cluster) => cluster.id)),
    [clusters],
  );
  ...
  const fetchBookings = async (page: number) => {
    try {
      ...
      const startDate = bookingStartFilter
        ? new Date(bookingStartFilter)
        : null;
      ...
      const response = await bookingApi.getAll({
        ...
        status:
          bookingStatusFilter.length > 0
            ? bookingStatusFilter.join(",")
            : undefined,
        pod_id:
          bookingPodFilter.length > 0 ? bookingPodFilter.join(",") : undefined,
  ...
  const fetchOrders = async (page: number) => {
    try {
      ...
      const response = await bookingOrderApi.getAll({
        ...
        status:
          orderStatusFilter.length > 0
            ? orderStatusFilter.join(",")
            : undefined,
        pod_ids:
          orderPodFilter.length > 0 ? orderPodFilter.join(",") : undefined,
        start_date: startDate ? startDate.toISOString() : undefined,
        end_date: endDate ? endDate.toISOString() : undefined,
      });
      ...
      toast.error(
        error?.response?.data?.message || "Failed to load booking orders",
      );
      setOrders([]);
  ...
  const openBookingDetail = async (bookingId: string) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to load booking detail",
      );
  ...
  const openRelatedBookingsByPod = async (podId: string) => {
    ...
    setRelatedBookings([]);
    setRelatedTitle(
      `Bookings in pod ${podMap.get(podId)?.code ?? compactId(podId)}`,
    );
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to load bookings by pod",
      );
  ...
  const openRelatedBookingsByOrder = async (orderId: string) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to load bookings by order",
      );
      setIsRelatedOpen(false);
    } finally {
      setIsRelatedLoading(false);
  ...
  const handleToggleCleanerAccess = async (booking: BookingItem) => {
    ...
    try {
      ...
      const updated = await bookingApi.setCleanerAccess(
        booking.id,
        nextAllowed,
      );
      ...
      toast.error(
        error?.response?.data?.message || "Failed to update cleaner access",
      );
    } finally {
  ...
  const openOrderDetail = async (orderId: string) => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to load booking order detail",
      );
  ...
  const handleCreateDamageBill = async () => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message || "Failed to create damage bill",
      );
    } finally {
  ...
  const applyFilters = () => {
    if (activeTab === "bookings") {
      ...
      setBookingStartFilter(
        draftBookingFilters.dateRange[0]
          ? dayjs(draftBookingFilters.dateRange[0]).format("YYYY-MM-DDTHH:mm")
          : "",
      );
      setBookingEndFilter(
        draftBookingFilters.dateRange[1]
          ? dayjs(draftBookingFilters.dateRange[1]).format("YYYY-MM-DDTHH:mm")
          : "",
      );
      setBookingPage(1);
    } else {
      ...
      setOrderStartFilter(
        draftOrderFilters.dateRange[0]
          ? dayjs(draftOrderFilters.dateRange[0]).format("YYYY-MM-DDTHH:mm")
          : "",
      );
      setOrderEndFilter(
        draftOrderFilters.dateRange[1]
          ? dayjs(draftOrderFilters.dateRange[1]).format("YYYY-MM-DDTHH:mm")
          : "",
      );
      setOrderPage(1);
    }
  ...
  const groupedBookings = useMemo(() => {
    ...
    return bookings.reduce((acc: any, booking: any) => {
      // Lấy ID của đơn hàng làm khóa gom nhóm
      const orderId = booking.order_id || "unknown-order";

      if (!acc[orderId]) {
        acc[orderId] = {
          // Lấy thông tin order từ booking đầu tiên tìm thấy
          order: booking.order || {
            id: orderId,
            status: "N/A",
            payable_total_price: 0,
          },
          items: [],
        };
      }

      acc[orderId].items.push(booking);
      return acc;
    }, {});
  }, [bookings]);
  ...
  return (
    <div className="p-6 lg:p-8 bg-gray-50 min-h-screen">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Monitor bookings and booking orders inside your assigned manager
            scope.
          </p>
        </div>
        ...
        >
          <RefreshCw
            className={`w-4 h-4 ${isScopeLoading || isBookingsLoading || isOrdersLoading ? "animate-spin" : ""}`}
          />
          Refresh
        </button>
      </div>
            ...
            {activeTab === "bookings" ? (
              <div className="min-w-[500px] flex-1 py-1">
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  {BOOKING_STATUSES.filter((s) => bookingCountsByStatus[s]).map(
                    (status) => (
                      <span
                        key={status}
                      ...
                      >
                        <span
                          className={`w-2 h-2 rounded-full ${bookingStatusBgColor(status)}`}
                        />
              ...
              <div className="min-w-[500px] flex-1 py-1">
                <p className="text-sm font-semibold text-gray-900 mb-1.5">
                  {orders.length} current page items
                </p>
                <div className="flex h-2.5 rounded-full overflow-hidden bg-gray-100 mb-1.5">
                  {BOOKING_ORDER_STATUSES.map((status) => {
                    ...
                    return percent > 0 ? (
                      <div
                        key={status}
                        className={orderStatusBgColor(status)}
                        style={{ width: `${percent}%` }}
                      />
                    ) : null;
                  })}
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  {BOOKING_ORDER_STATUSES.filter(
                    (s) => orderCountsByStatus[s],
                  ).map((status) => (
                    <span
                      key={status}
                      className="inline-flex items-center gap-1 text-xs text-gray-600"
                    >
                      <span
                        className={`w-2 h-2 rounded-full ${orderStatusBgColor(status)}`}
                      />
      ...
      {activeTab === "bookings" ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="text-sm text-gray-500">
                {bookingPagination.total_items} item(s)
              </span>
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {isBookingsLoading ? (
                    ...
                    <tr>
                      ...
                      >
                        Không tìm thấy lượt đặt phòng nào trong phạm vi này.
                      </td>
                    </tr>
                  ) : (
                    Object.values(groupedBookings).map((group: any) => (
                      <React.Fragment key={group.order?.id || Math.random()}>
                        <tr className="bg-slate-100 border-t-2 border-slate-200 ">
                          {/* Cột 1: Booking ID -> Hiển thị icon & Mã Order */}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col">
                                ...
                                <span className="text-sm font-bold text-slate-900 leading-none mt-1">
                                  {compactId(group.order?.id)}
                                </span>
                              </div>
                            </div>
                          </td>

                          {/* Cột 2: Pod -> Trống hoặc hiển thị Số lượng Pod */}
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter leading-none">
                                Quy mô
                              ...
                              <span className="text-xs font-bold text-slate-600 mt-1">
                                {group.items?.length || 0} Pods
                              </span>
                            </div>
                          </td>

                          {/* Cột 3: Check-in -> Trống hoặc bỏ qua */}
                          <td className="px-6 py-4"></td>

                          {/* Cột 4: Time -> Trống hoặc bỏ qua */}
                          <td className="px-6 py-4"></td>

                          {/* Cột 5: Status -> Hiển thị Trạng thái Thanh toán (Thẳng hàng với cột Status của Booking) */}
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              ...
                              {/* Sử dụng hàm helper để lấy cả label và màu sắc */}
                              {(() => {
                                const { label, classes } = getOrderStatusDetail(
                                  group.order?.status,
                                );
                                return (
                                  <span
                                    className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold w-fit border shadow-sm ${classes}`}
                                  >
                                    {/* Chấm tròn nhỏ trạng thái */}
                                    <div
                                      className={`w-1 h-1 rounded-full ${
                                        group.item?.status === "PAID"
                                          ? "bg-emerald-500"
                                          : group.order?.status === "PENDING"
                                            ? "bg-amber-500"
                                            : group.order?.status === "CANCEL"
                                              ? "bg-rose-500"
                                              : "bg-current"
                                      }`}
                                    />
                          ...
                          {/* Cột 6: Cleaner Access -> Hiển thị Tổng tiền (Vì cột này thường rộng, đủ chỗ cho số tiền) */}
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter leading-none">
                                Tổng cộng
                              </span>
                              <span className="text-sm font-black text-blue-700 mt-1">
                                {(
                                  group.order?.final_total_price || 0
                          ...
                          {/* Cột 7: Actions -> Nút xem chi tiết Order */}
                          <td className="px-6 py-4 text-right">
                            <button
                              type="button"
                              onClick={() =>
                                openRelatedBookingsByOrder(group.order?.id)
                              }
                              className="p-1.5 bg-white hover:bg-slate-100 border border-slate-200 rounded text-slate-400 hover:text-blue-600 transition-colors"
                        ...
                        {/* --- CÁC DÒNG BOOKING CON TRONG ORDER --- */}
                        {group.items?.map((booking: any) => {
                          const pod = podMap?.get(booking.pod_id);
                          const isUpdating =
                            updatingCleanerAccessId === booking.id;
                          ...
                          return (
                            ...
                            >
                              {/* ID Booking & Checklist status */}
                              <td className="px-6 py-5 pl-12 relative">
                                <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-slate-200" />
                                <div className="absolute left-6 top-1/2 -translate-y-1/2 w-4 h-0.5 bg-slate-200" />

                                <div className="flex items-center gap-4 relative">
                                  <div
                                    className={`w-3 h-3 rounded-full ring-4 ring-white shadow-sm transition-colors ${
                                      booking.is_checklist_completed
                                        ? "bg-emerald-500"
                                        : "bg-slate-300"
                                    }`}
                                    title={
                                  ...
                                  <div className="flex items-center  ">
                                    <p className="font-bold text-slate-800 group-hover:text-blue-600 transition-colors tracking-tight m-0">
                                      {booking.is_checklist_completed
                                        ? "Khách đã đối soát đồ"
                                        : "Khách chưa đối soát"}
                                    </p>
                                  </div>
                                </div>
                              ...
                              <td className="px-6 py-5">
                                <div className="flex flex-col">
                                  <p className="font-bold text-slate-700 uppercase text-xs tracking-wide">
                                    {pod?.code || booking.pod?.code || "—"}
                                  </p>
                                  <p className="text-xs text-slate-500 italic mt-0.5">
                                    {pod?.name || booking.pod?.name}
                                  </p>
                                </div>
                              </td>

                              {/* Trạng thái Check-in thực tế */}
                              <td className="px-6 py-5">
                                {(() => {
                                  const info = getCheckInStateDetail(
                                  ...
                                  return (
                                    <div className="flex flex-col gap-1.5">
                                      <div
                                        className={`inline-flex items-center gap-1.5 px-2 py-1 rounded border w-fit ${info.classes}`}
                                      >
                                        <span className="font-black uppercase text-[10px] leading-none tracking-tight">
                                          {info.label}
                                        </span>
                                      </div>

                                      <span className="text-[10px] text-slate-400 italic ml-1 min-h-[14px]">
                                        {booking.checked_in_at
                                          ? `Vào lúc: ${new Date(booking.checked_in_at).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}`
                                          : info.description}
                                      </span>
                                    </div>
                                  );
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <div className="flex flex-col gap-1">
                                      <div className="flex items-center gap-1.5 font-bold text-slate-700 tracking-tight">
                                        <span>{timeRange}</span>
                                      </div>
                              ...
                              <td className="px-6 py-5">
                                <span
                                  className={`px-2.5 py-1 rounded-full text-[10px] font-black tracking-widest shadow-sm border ${bookingStatusBadgeClass(booking.status)}`}
                                >
                                  {booking.status}
                                </span>
                              </td>
                              ...
                              <td className="px-6 py-5 text-right">
                                <button
                                  ...
                                  className="p-2 bg-white rounded-xl border border-slate-200 text-slate-400 hover:text-blue-600 hover:border-blue-200 hover:shadow-lg transition-all"
                                  title="Xem chi tiết"
                                >
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
              <p className="text-xs text-gray-500">
                Page {bookingPagination.current_page} /{" "}
                {bookingPagination.total_pages}
              </p>
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={
                    bookingPagination.current_page <= 1 || isBookingsLoading
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    setBookingPage((prev) =>
                      Math.min(prev + 1, bookingPagination.total_pages || 1),
                    )
                  }
                  disabled={
                    bookingPagination.current_page >=
                      bookingPagination.total_pages || isBookingsLoading
                  }
                  className="px-3 py-1.5 rounded-md border border-gray-200 text-sm text-gray-700 disabled:opacity-50"
                >
                  Next
        ...
        <>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-base font-semibold text-gray-900">
                Scoped Booking Orders
              </h2>
              <span className="text-sm text-gray-500">
                {orderPagination.total} item(s)
              </span>
            </div>
                  ...
                  {isOrdersLoading ? (
                    ...
                    <tr>
                      <td
                        ...
                        className="px-6 py-12 text-center text-gray-400"
                      >
                        No booking orders found in your scope.
                      </td>
                    </tr>
                  ) : (
                    orders.map((order) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 align-top">
                          <span
                            className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${orderStatusBadgeClass(order.status)}`}
                          >
                            {order.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 align-top text-gray-700">
                          {order.bookings_count ?? 0}
                        </td>
                        <td className="px-6 py-4 align-top text-gray-700">
                          {formatMoney(order.final_total_price)}
                        </td>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
              <p className="text-xs text-gray-500">
                ...
                {Math.max(orderPagination.pages, 1)}
              </p>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                ...
                <button
                  ...
                  onClick={() =>
                    setOrderPage((prev) =>
                      Math.min(prev + 1, Math.max(orderPagination.pages, 1)),
                    )
                  }
                  disabled={
                    orderPagination.page >= orderPagination.pages ||
                    isOrdersLoading
                  }
                  className="px-3 py-1.5 rounded-md border border-gray-200 text-sm text-gray-700 disabled:opacity-50"
                >
      ...
      >
        ...
        />
        <div
          className={`absolute right-0 top-0 h-full w-full max-w-4xl overflow-hidden bg-white shadow-2xl border-l border-gray-200 transform transition-transform duration-300 lg:right-4 lg:top-4 lg:bottom-4 lg:h-auto lg:w-[calc(100%-2rem)] lg:border lg:rounded-xl flex flex-col ${isFilterPanelOpen ? "translate-x-0" : "translate-x-[110%]"}`}
          role="dialog"
          aria-modal="true"
        >
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Filter {activeTab} by status and pod.
              </p>
            </div>
            <button
              type="button"
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() =>
                        setDraftBookingFilters((prev) => ({
                          ...prev,
                          status: toggleArrayFilter(
                            prev.status,
                            "all",
                            BOOKING_STATUSES.length,
                          ),
                        }))
                      }
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-colors border ${draftBookingFilters.status.length === 0 ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}
                    >
                      {draftBookingFilters.status.length === 0 && (
                  ...
                  <div className="border border-gray-200 rounded-xl shadow-sm bg-white custom-calendar w-full overflow-hidden">
                    <div className="w-full p-4">
                      <DatePicker
                        ...
                        startDate={
                          draftBookingFilters.dateRange[0] || undefined
                        }
                      ...
                      onClick={() =>
                        setDraftOrderFilters((prev) => ({
                          ...
                          status: toggleArrayFilter(
                            prev.status,
                            "all",
                            BOOKING_ORDER_STATUSES.length,
                          ),
                        }))
                      }
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-colors border ${draftOrderFilters.status.length === 0 ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}
                    >
                      {draftOrderFilters.status.length === 0 && (
                        <Check className="w-4 h-4 inline-block mr-1.5 -ml-0.5" />
                      )}
                      All
                    </button>
      ...
      <div
        className={`fixed inset-0 z-50 ${isBookingDetailOpen ? "" : "pointer-events-none"}`}
        aria-hidden={!isBookingDetailOpen}
      >
        <div
              ...
              <div className="space-y-6">
                {(() => {
                  ...
                  if (selectedBooking.status === "IN_USE") {
                    ...
                    title = "In Use";
                    desc = "Booking is currently active.";
                    ...
                    title = "Booked";
                    desc = "Booking is confirmed and upcoming.";
                    ...
                    title = "Completed";
                    desc = "Booking has been fulfilled.";
                    ...
                    iconBg =
                      "bg-white text-gray-500 shadow-sm border border-gray-50";
                    title = selectedBooking.status;
                    desc = "Booking status pending.";
                  ...
                  <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">
                        Booking đã hoàn tất, không thể tiếp tục tiếp nhận hành
                        động
                      </p>
                      <p className="text-xs text-gray-500 mt-0.5">
                        Mọi thao tác quản lý đã bị vô hiệu hóa.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex flex-col text-sm">
                  ...
                  <div className="flex justify-between items-center py-4 border-b border-gray-100">
                    ...
                    <span className="font-medium text-gray-900">
                      {selectedBooking.checkin_state ?? "—"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-gray-100">
                    <span className="text-gray-500">Cleaner Access</span>
                    <span className="font-medium text-gray-900">
                      {selectedBooking.cleaner_access_allowed
                        ? "Allowed"
                        : "Disabled"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-gray-100">
                    ...
                    <span className="font-medium text-gray-900">
                      {formatMoney(selectedBooking.base_price)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-gray-100">
            ...
            {relatedBookings.map((booking) => (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                  ...
                  <span
                    className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${bookingStatusBadgeClass(booking.status)}`}
                  >
                    {booking.status}
                  </span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3 text-sm text-gray-600">
                  <p>
                    ...
                    {podMap.get(booking.pod_id)?.code ??
                      compactId(booking.pod_id)}
                  </p>
                  <p>
                    <span className="text-gray-500">Order:</span>{" "}
                    {compactId(booking.order_id)}
                  </p>
                  <p>
      ...
      <div
        className={`fixed inset-0 z-50 ${isOrderDetailOpen ? "" : "pointer-events-none"}`}
        aria-hidden={!isOrderDetailOpen}
      >
        <div
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6">
            {isOrderDetailLoading ? (
              <div className="py-8 text-center text-gray-500">
                Loading booking order detail...
              </div>
            ) : !selectedOrderDetail ? (
              <div className="py-8 text-center text-gray-500">
                No booking order data.
              </div>
            ) : (
              <div className="space-y-6">
                {(() => {
                  ...
                  if (selectedOrderDetail.order.status === "PAID") {
                    ...
                    selectedOrderDetail.order.status === "CANCEL" ||
                    selectedOrderDetail.order.status === "FULLY_CANCELLED"
                  ) {
                    bg = "from-rose-50/80 to-white border-rose-100";
                    iconBg =
                      "bg-white text-rose-500 shadow-sm border border-rose-50";
                    ...
                    iconBg =
                      "bg-white text-gray-500 shadow-sm border border-gray-50";
                    title = selectedOrderDetail.order.status;
                    desc = "Order status details.";
                ...
                {selectedOrderDetail.order.damage_payment_status &&
                  selectedOrderDetail.order.damage_payment_status !==
                    "NO_INCIDENT" && (
                    <div
                      className={`rounded-xl overflow-hidden shadow-sm border ${selectedOrderDetail.order.damage_payment_status === "PAID" ? "border-emerald-200" : "border-rose-200"}`}
                    >
                      <div
                        className={`px-4 py-3 border-b flex items-center justify-between ${selectedOrderDetail.order.damage_payment_status === "PAID" ? "bg-emerald-50 border-emerald-100" : "bg-rose-50 border-rose-100"}`}
                      >
                        <h3
                          className={`font-semibold flex items-center gap-2 ${selectedOrderDetail.order.damage_payment_status === "PAID" ? "text-emerald-800" : "text-rose-800"}`}
                        >
                      ...
                      <div className="p-4 bg-white flex justify-between items-center">
                        <div>
                          <p className="text-sm text-gray-600">
                            Trạng thái thanh toán hóa đơn đền bù của đơn hàng
                            này.
                          </p>
                        </div>
                        {selectedOrderDetail.order.damage_payment_status !==
                ...
                <div className="flex flex-col text-sm">
                  ...
                  <div className="flex justify-between items-center py-4 border-b border-gray-100">
                    ...
                    <span className="font-medium text-gray-900">
                      {selectedOrderDetail.order.user?.name || "Unknown User"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-gray-100">
                    <span className="text-gray-500">User Email</span>
                    <span className="font-medium text-gray-900">
                      {selectedOrderDetail.order.user?.email || "—"}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-gray-100">
                    <span className="text-gray-500">Final Total</span>
                    <span className="font-medium text-gray-900">
                      {formatMoney(selectedOrderDetail.order.final_total_price)}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-sm font-semibold text-gray-900 mb-3">
                    Bookings in this order
                  </h3>
                  {selectedOrderDetail.bookings.length === 0 ? (
                    <p className="text-sm text-gray-500">
                      No bookings visible in your manager scope.
                    </p>
                  ) : (
                    <div className="space-y-3 max-h-[42vh] overflow-auto pr-1">
                      {selectedOrderDetail.bookings.map((booking) => (
                        <div
                          key={booking.id}
                          className="border border-gray-200 rounded-lg p-3 text-sm flex items-center justify-between"
                        >
                ...
                <div className="border-t border-gray-100 pt-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-semibold text-gray-900">
                      Sự cố của đơn hàng này
                    </h3>
                    <button
                      onClick={handleCreateDamageBill}
                      disabled={
                        isCreatingDamageBill ||
                        selectedOrderIncidents.some(
                          (i) => i.status === "PENDING",
                        ) ||
                        selectedOrderIncidents.filter(
                          (i) => i.status === "RESOLVED",
                        ).length === 0 ||
                        isReadOnly ||
                        (selectedOrderDetail.order.damage_payment_status &&
                    ...
                    >
                      {isCreatingDamageBill
                        ...
                        : selectedOrderDetail.order.damage_payment_status &&
                            selectedOrderDetail.order.damage_payment_status !==
                              "NO_INCIDENT"
                          ? "Đã tạo hóa đơn đền bù"
                          : "Tạo hóa đơn đền bù cho đơn hàng"}
                    </button>
                  </div>

                  {isOrderIncidentsLoading ? (
                    <div className="flex justify-center py-4 text-gray-400">
                      Đang tải sự cố của đơn hàng...
                    </div>
                  ) : selectedOrderIncidents.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-gray-200 p-4 text-sm text-gray-500">
                      Không có sự cố nào khác trong đơn hàng này.
                    </div>
                  ) : (
                      ...
                      {selectedOrderIncidents.map((inc) => (
                        ...
                        >
                          ...
                          <div className="flex flex-col items-end gap-2 ml-4">
                            <div className="flex items-center gap-2">
                              <span
                                className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold ${incidentStatusBadgeClass(inc.status)}`}
                              >
                                {translateIncidentStatus(inc.status)}
                              </span>
                              <span
                                className={`inline-flex rounded-md px-2 py-1 text-[10px] uppercase tracking-wider ${incidentSeverityBadgeClass(inc.severity)}`}
                              >
                                {translateIncidentSeverity(inc.severity)}
                              </span>
                            </div>
                            {inc.total_amount_value !== undefined && (
                              <span className="text-sm font-bold text-gray-900">
                                {formatMoney(inc.total_amount_value)}
                              </span>
                            )}
                          </div>
                        </div>
