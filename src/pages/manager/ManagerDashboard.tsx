...
const getDateRange = (
  range: RangeOption,
): { from: Date; to: Date; groupBy: DashboardFilters["groupBy"] } => {
  const now = new Date();
...
const TrendBadge: React.FC<{ value?: number; label?: string }> = ({
  value,
  label,
}) => {
  ...
  return (
    <div
      className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-bold ${colorClass} mb-1 shadow-sm border border-current border-opacity-10`}
      title={label}
    >
...
const SectionTitle: React.FC<{
  ...
  <div className="flex items-center gap-2 ">
    <div>
      {icon && <span className="text-gray-400 inline-flex">{icon}</span>}
    </div>
    <div className="flex items-center m-0 p-0">
      <h2 className="text-xl font-semibold text-gray-800 whitespace-nowrap m-0 p-0">
        {children}
      </h2>
    </div>
  </div>
...
export const ManagerDashboard = () => {
  ...
  const [summaryRawData, setSummaryRawData] = useState<
  ...
  const fetchSummaryDashboard = async (
    ...
    customDates?: [dayjs.Dayjs, dayjs.Dayjs] | null,
  ) => {
    try {
      ...
      if (selectedRange === "custom" && customDates) {
        ...
        rangeQuery = getDateRange(
          selectedRange === "custom" ? "month" : selectedRange,
        );
      }
      ...
      setError(
        fetchError?.response?.data?.message || "Failed to load summary data",
      );
  ...
  const fetchChartDashboard = async (
    selectedRange: RangeOption,
    customDates?: [dayjs.Dayjs, dayjs.Dayjs] | null,
  ) => {
    try {
      ...
      setError(
        fetchError?.response?.data?.message || "Failed to load chart data",
      );
  ...
  const { summaryData, listData } = useMemo(() => {
    ...
    const scopedBookings = summaryRawData.bookings.list.filter((booking) =>
      scopedPodIds.has(String(booking.pod_id)),
    );
    const scopedIncidents = summaryRawData.incidents.list.filter((incident) =>
      scopedPodIds.has(String(incident.pod_id)),
    );

    const bookingsByStatus = scopedBookings.reduce<Record<string, number>>(
      (acc, booking) => {
    ...
    const podMap = new Map<string, DashboardPod>(
      scopedPods.map((pod) => [pod.id, pod]),
    );
    const clusterMap = new Map<string, string>(
      scopedClusters.map((c) => [c.id, c.name]),
    );
    ...
    return {
      summaryData: {
        ...
        revenue: {
          totalInRange:
            summaryRawData.summary.revenueInRange ??
            summaryRawData.bookings.revenue?.total ??
            0,
        },
      ...
      listData: {
        latestBookings: [...scopedBookings]
          ...
          .map((booking) => {
            ...
            const clusterName = pod?.cluster_id
              ? clusterMap.get(String(pod.cluster_id))
              : "-";
            ...
            return {
              ...
              userName:
                (booking.user as { name?: string } | undefined)?.name ?? "-",
  ...
  const chartData = useMemo(() => {
    ...
    const scopedPodIds = new Set(scopedPods.map((pod) => pod.id));
    const scopedBookings = chartRawData.bookings.list.filter((booking) =>
      scopedPodIds.has(String(booking.pod_id)),
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Tổng quan hoạt động trong khu vực bạn quản lý.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <RefreshCw
              className={`w-4 h-4 ${isPageLoading ? "animate-spin" : ""}`}
            />
            Làm mới
          </button>
        </div>
      ...
      {!isPageLoading && (
        <>
          <div className="flex items-center justify-between mb-3 mt-6">
            <SectionTitle icon={<Package className="w-7 h-7" />}>
              Tổng Quan Hoạt Động
            </SectionTitle>
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                >
                  ...
                  {RANGE_OPTIONS.find((o) => o.value === summaryRange)?.label ??
                    "Tháng này"}
                  <ChevronDown
                    className={`w-4 h-4 transition-transform ${showSummaryRangePicker ? "rotate-180" : ""}`}
                  />
                </button>
                {showSummaryRangePicker && (
                  <>
                    <div
                      ...
                      onClick={() => setShowSummaryRangePicker(false)}
                    />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <SummaryCard
              ...
              badge={
                <TrendBadge
                  value={summaryData.comparison?.bookingsChange}
                  label="So với kỳ trước"
                />
              }
              extra={Object.entries(summaryData.bookings.byStatus).map(
                ([status, count]) => (
                  <div
                    key={status}
            ...
            <SummaryCard
              ...
              badge={
                <TrendBadge
                  value={summaryData.comparison?.ordersChange}
                  label="So với kỳ trước"
                />
              }
            />
            ...
            <SummaryCard
              ...
              value={new Intl.NumberFormat("vi-VN", {
                style: "currency",
                currency: "VND",
              }).format(summaryData.revenue.totalInRange)}
              icon={<TrendingUp className="w-5 h-5 text-purple-600" />}
              iconBg="bg-purple-50"
              badge={
                <TrendBadge
                  value={summaryData.comparison?.revenueChange}
                  label="So với kỳ trước"
                />
              }
            />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col">
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    ...
                    >
                      <RefreshCw className="w-3 h-3 text-gray-400 group-hover:rotate-180 transition-transform" />
                      {RANGE_OPTIONS.find((o) => o.value === chartRange)
                        ?.label ?? "Tuần này"}
                      <ChevronDown
                        className={`w-4 h-4 transition-transform ${showChartRangePicker ? "rotate-180" : ""}`}
                      />
                    </button>
                    {showChartRangePicker && (
                      <>
                        <div
                          className="fixed inset-0 z-10"
                          onClick={() => setShowChartRangePicker(false)}
                        />
                        <div className="absolute right-0 mt-1 w-32 bg-white border border-gray-200 rounded-lg shadow-xl z-20 overflow-hidden animate-in fade-in zoom-in duration-100">
                          {RANGE_OPTIONS.map((opt) => (
                            <button
              ...
              {chartData.revenueTrend.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8 my-auto">
                  Không có dữ liệu trong thời gian này
                </p>
              ) : (
                <div className="flex-1">
                  <LineChart
                    labels={chartData.revenueTrend.map((point) =>
                      mapDateLabelToVietnamese(point.label, chartData.groupBy),
                    )}
                    revenue={chartData.revenueTrend.map(
                      (point) => point.amount,
                    )}
                    orders={chartData.revenueTrend.map(
                      (point) => point.orders ?? 0,
                    )}
                  />
                </div>
              )}
            ...
            <div className="lg:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              {chartData.bookingStatusRates.length === 0 ? (
                ...
                <DoughnutChart
                  labels={chartData.bookingStatusRates.map(
                    (item) => item.status,
                  )}
                  values={chartData.bookingStatusRates.map(
                    (item) => item.count,
                  )}
                  colors={chartData.bookingStatusRates.map((item, index) =>
                    statusColor(item.status, index),
                  )}
                />
              )}
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {listData.latestBookings.length > 0 ? (
                      listData.latestBookings.map((booking) => (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {listData.latestIncidents.length > 0 ? (
                      listData.latestIncidents.map((incident) => (
                      ...
                      <tr>
                        <td
                          ...
                          className="px-6 py-8 text-center text-gray-400"
                        >
                          Không có sự cố trong thời gian này
                        </td>
                      </tr>
                    )}
                  </tbody>
